/* eslint-disable */
var flowy = function(canvas, grab, release, snapping, rearrange, spacing_x, spacing_y, onExistingSnap) {
    if (!grab) {
        grab = function() {};
    }
    if (!release) {
    ...
    flowy.load = function() {
        ...
        // ATS - CHANGES to delete block by ID
        ...
        flowy.endDrag = function(event) {
            ...
            if (event.which != 3 && (active || rearrange)) {
                ...
                if (parseInt(drag.querySelector(".blockid").value) === 0 && rearrange) {
                    // ATS - This place will be called when the root block is moved around
                    ...
                    // ATS - This place will be called when the root block was able to place successfully inside the canvas
                    ...
                    // ATS - This place will be called when root element was dragged outside the canvas
                    canvas_div.appendChild(document.querySelector(".indicator"));
                    drag.parentNode.removeChild(drag);
                } else if (active) {
                    // ATS - This place will be called when a new block is snapped to the existing block
                    ...
                    for (var i = 0; i < blocks.length; i++) {
                        if (xpos >= blocks.filter(a => a.id == blocko[i])[0].x - (blocks.filter(a => a.id == blocko[i])[0].width / 2) - paddingx && xpos <= blocks.filter(a => a.id == blocko[i])[0].x + (blocks.filter(a => a.id == blocko[i])[0].width / 2) + paddingx && ypos >= blocks.filter(a => a.id == blocko[i])[0].y - (blocks.filter(a => a.id == blocko[i])[0].height / 2) && ypos <= blocks.filter(a => a.id == blocko[i])[0].y + blocks.filter(a => a.id == blocko[i])[0].height) {
                            // ATS - This place will be called when block is moved but was able to attach to some block.
                            active = false;
                            drag.classList.remove('dragging');
                            // ATS CHANGES - Custom condition to check for snap of existing blocks
                            if (
                              onExistingSnap(
                                drag,
                            ...
                            if (beforeDelete(drag, blocks.filter(id => id.id == blocko[i])[0])) {
                                // ATS - this place will be called only if rearrange method is 
                                // passed while initializing defaults to false.
                                active = false;
                                drag.classList.remove("dragging");
                                snap(drag, blocko.indexOf(prevblock), blocko);
                                break;
                            } else {
                                // ATS - this place will remove the branch if the rearrage method returns false 
                                // and element was not able to attach to any block.
                                rearrange = false;
                                blockstemp = [];
                                active = false;
                                canvas_div.appendChild(document.querySelector(".indicator"));
        ...
        function touchblock(event) {
            ...
            if (hasParentClass(event.target, "block")) {
                ...
                if (event.type !== "mouseup" && hasParentClass(event.target, "block") && 
                    // ATS CHANGES - added this below condition so that root block cannot be moved.
                    parseInt(theblock.querySelector(".blockid").value) !== 0) {
                    if (event.which != 3) {
                        if (!active && !rearrange) {
                            dragblock = true;
                            drag = theblock;
        ...
        flowy.moveBlock = function(event) {
            ...
            if (active || rearrange) {
                // To make canvas auto scrollable
